...
<u>NOTE</u>: GCS bucket names must be globally unique. Update the GCS_BUCKET accordingly.
...
<u>NOTE</u>: GCS bucket names must be globally unique. Update the bucket name in `variables.tf` accordingly.
```
